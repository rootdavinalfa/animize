...
class StreamActivity : AppCompatActivity() {
    ...
    fun onEvent(error: PlayerBusError) {
        ...
        texterror.text = error.geterror()
        println("ERROR:" + error.geterror())
    }

    fun getVideo() {
        println("KLKLKLKL " + idanim)
    ...
    private fun showVideo(video: JSONArray) {
        ...
        try {
            for (i in 0 until video.length()) {
                ...
                detailsEpisode!!.text = "Episode ${epi}"
        ...
        val epe = getString(R.string.episode_text).toString() + ": " + modeldata!![0].episode + " " + getString(R.string.string_of) + " " + modeldata!![0].total_ep_anim
    ...
    private fun playerConstant() {
        ...
        playerView!!.useArtwork = true
        playerView!!.defaultArtwork = resources.getDrawable(R.drawable.ic_astronaut)
    ...
    private fun changeStar(){
        var status = ""
        GlobalScope.run {
            val pkganim = modeldata!![0].pack
            val starred =  packageStarDBHelper!!.isStarred(pkganim)
            status = if (starred) {
    ...
    private fun updateRecent() {
        if (PlayerManager.getService() != null) {
            if (Objects.requireNonNull(PlayerManager.getService().exoPlayer)!!.playbackState == Player.STATE_READY || PlayerManager.getService().exoPlayer!!.playbackState == Player.STATE_BUFFERING) {
